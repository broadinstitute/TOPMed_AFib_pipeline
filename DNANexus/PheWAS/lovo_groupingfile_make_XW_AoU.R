#!/usr/bin/env Rscript

#### binary traits
args=(commandArgs(TRUE))
group1=as.character(args[1])
group2=as.character(args[2])
group3=as.character(args[3])
gene=as.character(args[4]) # Ensembl ID

library(data.table)
# Group file 1 (LOF)
g1 <- get(load(group1))
g1$varid <- paste0(g1$chr, ":", g1$pos, ":", g1$ref, ":", g1$alt)
g1 <- g1[which(grepl(gene, g1$group_id)), ]

# Group file 2 (LOF+missense)
g2 <- get(load(group2))
g2$varid <- paste0(g2$chr, ":", g2$pos, ":", g2$ref, ":", g2$alt)
g2 <- g2[which(grepl(gene, g2$group_id)), ]

# Group file 3 (ultra-rare missense)
g3 <- get(load(group3))
g3$varid <- paste0(g3$chr, ":", g3$pos, ":", g3$ref, ":", g3$alt)
g3 <- g3[which(grepl(gene, g3$group_id)), ]

# Find unique variants across annotations
varz <- unique(c(g1$varid, g2$varid, g3$varid))

# Make LOVO groupings for the gene
lovo_g1 <- g1
lovo_g2 <- g2
lovo_g3 <- g3
for(var in varz){
    
    if(nrow(g1)>0){
        inter1 <- g1
        if(var %in% inter1$varid){
            inter1 <- inter1[-(which(inter1$varid==var)), ]
        }
        try(inter1$group_id <- paste0(inter1$group_id, ":LOVO", var))
        try(lovo_g1 <- rbind(lovo_g1, inter1))
    }
    
    if(nrow(g2)>0){
        inter2 <- g2
        if(var %in% inter2$varid){
            inter2 <- inter2[-(which(inter2$varid==var)), ]
        }
        try(inter2$group_id <- paste0(inter2$group_id, ":LOVO", var))
        try(lovo_g2 <- rbind(lovo_g2, inter2))
    }
    
    if(nrow(g3)>0){
        inter3 <- g3
        if(var %in% inter3$varid){
            inter3 <- inter3[-(which(inter3$varid==var)), ]
        }
        try(inter3$group_id <- paste0(inter3$group_id, ":LOVO", var))
        try(lovo_g3 <- rbind(lovo_g3, inter3))
    }

}

save(lovo_g1, file='LOVO_group1.RData')
save(lovo_g2, file='LOVO_group2.RData')
save(lovo_g3, file='LOVO_group3.RData')
