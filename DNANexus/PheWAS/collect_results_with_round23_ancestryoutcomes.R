...
if(!(all(file.exists(maf0.001_files)) & all(file.exists(maf0.00001_files)) & all(file.exists(maf0.01_files)))){
    ...
    rawassoc_res <- inter
    write.table(rawassoc_res, file=paste0('../summary_results_phewas_all_tests_phecode', num, '.tsv'), col.names=T, row.names=F, quote=F, sep='\t', append=F)
    ...
    write.table(inter1, file=paste0('../summary_results_phewas_cauchy_phecode', num, '.tsv'), col.names=T, row.names=F, quote=F, sep='\t', append=F)
